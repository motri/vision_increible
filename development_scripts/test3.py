...
class ExerciseApp:
    ...
    def process_frame(self, frame):
        ...
        # Draw stick figure
        connections = [("ankle", "knee"), ("knee", "hip"), ("hip", "shoulder")]
        ...
        # Repetition detection logic for squat
        if self.start_tracking and "knee" in joints and "hip" in joints:
            knee_y, hip_y = joints["knee"][1], joints["hip"][1]

            # Bottom position detected (knee below hip)
            if knee_y > hip_y + 50 and not self.at_bottom:
                self.at_bottom = True

            # Top position detected (knee back above hip)
            elif knee_y < hip_y - 30 and self.at_bottom:
                self.rep_count += 1
